...
class RvneyoChatScreen extends StatefulWidget {
  final String userId;
  final String userName;
  final String userAvatar;

  const RvneyoChatScreen({
    super.key,
  ...
  @override
  State<RvneyoChatScreen> createState() => _RvneyoChatScreenState();
}

class _RvneyoChatScreenState extends State<RvneyoChatScreen> {
  List<_RvneyoChatMessage> _messages = [];
  ...
  Future<String> _getMediaDirectory() async {
    ...
    final mediaDir = Directory('${dir.path}/rvneyo_chat_media');
  ...
  Future<void> _loadMessages() async {
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/rvneyo_chat_history_${widget.userId}.json');
    if (await file.exists()) {
      try {
        ...
        setState(() {
          _messages = jsonList.map((e) => _RvneyoChatMessage.fromJson(e)).toList();
        });
  ...
  void _sendMessage() {
    ...
    setState(() {
        _messages.add(_RvneyoChatMessage(
        text: text,
        isMe: true,
        time: _getTimeStamp(),
          type: _RvneyoChatMessageType.text,
      ));
    });
  ...
  Future<void> _sendImage(File imageFile) async {
    try {
      ...
      final fileName = 'rvneyo_image_${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      setState(() {
        _messages.add(_RvneyoChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _RvneyoChatMessageType.image,
        ));
      });
      _scrollToBottom();
      _saveMessages();
  ...
  Future<void> _sendVoice(String audioPath, Duration duration) async {
    try {
      final mediaDir = await _getMediaDirectory();
      final fileName = 'rvneyo_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
      ...
      setState(() {
        _messages.add(_RvneyoChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _RvneyoChatMessageType.audio,
        ));
      });
      _scrollToBottom();
      _saveMessages();
    ...
    try {
      if (_isRecording) {
        ...
        if (await _voiceRecorder.hasPermission()) {
          ...
          final filePath = '${dir.path}/rvneyo_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
...
enum _RvneyoChatMessageType { text, image, audio }

class _RvneyoChatMessage {
  final String? text;
  ...
  final bool isMe;
  final String time;
  final _RvneyoChatMessageType type;
  ...
  static _RvneyoChatMessage fromJson(Map<String, dynamic> json) => _RvneyoChatMessage(
    text: json['text'],
    ...
    isMe: json['isMe'] ?? true,
    time: json['time'] ?? '',
    type: _RvneyoChatMessageType.values.firstWhere(
      (e) => e.name == json['type'],
      orElse: () => _RvneyoChatMessageType.text,
    ),
  );
}

class _RvneyoChatBubble extends StatefulWidget {
  final _RvneyoChatMessage message;
  final void Function(File file)? onImageTap;
  const _RvneyoChatBubble({required this.message, this.onImageTap});

  @override
  State<_RvneyoChatBubble> createState() => _RvneyoChatBubbleState();
}

class _RvneyoChatBubbleState extends State<_RvneyoChatBubble> {
  ...
  void initState() {
    super.initState();
    if (widget.message.type == _RvneyoChatMessageType.audio) {
  ...
  Future<void> _togglePlayPause() async {
    try {
      ...
      final dir = await getApplicationDocumentsDirectory();
      final absPath = '${dir.path}/rvneyo_chat_media/${msg.audioPath}';
  ...
  Widget build(BuildContext context) {
    ...
    if (msg.type == _RvneyoChatMessageType.audio && msg.audioPath != null) {
      ...
      return Align(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: msg.isMe ? null : Border.all(color: AppTheme.primaryColor.withOpacity(0.2), width: 1),
            boxShadow: [
              BoxShadow(
                color: msg.isMe 
    ...
    if (msg.type == _RvneyoChatMessageType.image && msg.imagePath != null) {
      return Align(
        alignment: msg.isMe ? Alignment.centerRight : Alignment.centerLeft,
        child: GestureDetector(
          onTap: () async {
            final dir = await getApplicationDocumentsDirectory();
            final absPath = '${dir.path}/rvneyo_chat_media/${msg.imagePath}';
                ...
                ClipRRect(
                  ...
                  child: FutureBuilder<Directory>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      final absPath = '${snapshot.data!.path}/rvneyo_chat_media/${msg.imagePath}';
                      return Image.file(
                        File(absPath),
                        width: 160,
                        height: 160,
...
class _RvneyoChatInputBar extends StatelessWidget {
