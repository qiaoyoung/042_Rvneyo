...
- (IBAction)onDoneBtnClick:(id)sender {
    ...
    if (_selectecContacts.count == 0) {
        [ValidateCompositionInterpolationToward showMessage:LangKey(@"main_activity_choose_least_one")];
        return;
    }
...
- (StructureCozyChat *)selectIndicatorView{
    ...
    _selectIndicatorView = [[StructureCozyChat alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, 64)];
...
- (FreeElevateBundleRetreat *)setGroupnameView{
    if(!_setGroupnameView){
        _setGroupnameView = [[FreeElevateBundleRetreat alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
