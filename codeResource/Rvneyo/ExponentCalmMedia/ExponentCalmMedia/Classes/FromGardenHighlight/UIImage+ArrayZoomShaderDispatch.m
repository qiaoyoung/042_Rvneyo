...
// __CLOSE_PRINT__
//
//  UIImage+ArrayZoomShaderDispatch
//  NIM
//
...
//: @implementation UIImage (ArrayZoomShaderDispatch)
@implementation UIImage (ArrayZoomShaderDispatch)
//: - (UIImage *)drawImageWithSize: (CGSize)size
- (UIImage *)cover: (CGSize)size
{
...
- (UIImage *)grace: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self constraint:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self constraint:suggestPixels];
    }
}
...
- (UIImage *)full
{
    //: CGFloat pixels = [[PastGesturePassive currentDevice] suggestImagePixels];
    CGFloat pixels = [[PastGesturePassive efficiency] providerStart];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self grace:pixels];
    //: return [image fixOrientation];
    return [image assignEstimate];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)scaleToSize:(CGSize)newSize
- (UIImage *)finish:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
...
- (UIImage *)constraint: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self finish:CGSizeMake(newSizeWidth, newSizeHeight)];
}
