...
// __CLOSE_PRINT__
//
//  NSData+ArrayZoomShaderDispatch.m
//  NIM
//
...
- (NSData *)sequenceDocument:(NSString *)key transitionPolicy:(NSString *)vector {
...
void plotUnitBe(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    //: char key_[256];
    char key_[256];
...
- (NSData *)method:(NSString *)key {
    ...
    plotUnitBe(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
...
- (NSData *)wisdom:(NSString *)key abort:(NSString *)vector {
    // 'key' should be 32 bytes for AES256, will be null-padded otherwise
    //: assert(key.length == 32);
    assert(key.length == 32);
    //: char keyPtr[kCCKeySizeAES256 + 1];
