...
@property (nonatomic, strong) FairElementalPaletteView *dropdownMenu;/** 下拉菜单 */

@property (nonatomic, strong) NSString *creatTeam;

@property (nonatomic, strong) SensePrimarySourcePackage *noticeView;
...
-(void)checkCreateTeam{
    ...
    [ConfigPrimalCozy getWithUrl:Server_check_createteam params:nil isShow:NO success:^(id responseObject) {
...
- (NSArray *)getMenuModelsArray {    
    ...
    FairElementalPaletteModel *menuModel1 = [FairElementalPaletteModel ff_DropDownMenuModelWithMenuItemTitle:activity menuItemIconName:@"ic_create_chat" menuBlock:^{
        if (_creatTeam.integerValue != 0) {
            [ValidateCompositionInterpolationToward showMessage:LangKey(@"please_contact_your_administrator")];
        }else{
    ...
    //菜单模型1
    @weakify(self);
    FairElementalPaletteModel *menuModel3 = [FairElementalPaletteModel ff_DropDownMenuModelWithMenuItemTitle:LangKey(@"fragment_contact_new_scan") menuItemIconName:@"ic_scan" menuBlock:^{
        @strongify(self);
        [self requestAuthorizationForVideo];
    }];
...
-(void)newGroupSyncRequest:(NSString *)groupName teamID:(NSString *)teamId{
    ...
    dict[@"name"] = groupName;
    dict[@"id"] = teamId;
    [ConfigPrimalCozy getWithUrl:Server_team_create params:dict isShow:NO success:^(id responseObject) {
...
{
    if(!_noticeView){
        _noticeView = [[SensePrimarySourcePackage alloc]initWithFrame:CGRectMake(0, SCREEN_STATUS_HEIGHT+140, SCREEN_WIDTH, SCREEN_SAFE_HEIGHT-140)];
...
{
    if(!_loadingView){
        _loadingView = [[DuplicateTowardSelectStore alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_policyView){
        _policyView = [[StarQuintessentialEquipmentHide alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
