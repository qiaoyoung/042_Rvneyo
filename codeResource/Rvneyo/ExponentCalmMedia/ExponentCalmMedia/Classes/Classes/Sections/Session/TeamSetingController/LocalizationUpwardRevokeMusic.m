...
- (void)updateTeamName{
    ...
    [self.groupnameView animationShow];
        ...
        self.groupnameView.speiceBackBlock = ^(NSString *groupName){
            ...
            [ConfigPrimalCozy getWithUrl:Server_team_modifyname params:dict isShow:NO success:^(id responseObject) {
                [self reloadData];
...
- (void)dismissTeam {
    ...
    [self.groupAlartView animationShow];
        ...
        self.groupAlartView.speiceBackBlock = ^(NSString *Name){
            @strongify(self)
            [self didDismissTeam];
            [ConfigPrimalCozy getWithUrl:Server_team_destory params:@{@"id":self.teamListManager.team.teamId} isShow:NO success:^(id responseObject) {
...
- (void)didOntransferWithLeave:(BOOL)isLeave {
    ...
    __block ContactSelectFinishBlock finishBlock =  ^(NSArray * memeber, NSString *name, UIImage *avater){
        ...
        [ConfigPrimalCozy getWithUrl:Server_team_modifyowner params:dict isShow:NO success:^(id responseObject) {
            [wself reloadData];
        } failed:^(id responseObject, NSError *error) {
...
{
    if(!_groupnameView){
        _groupnameView = [[InteractorEfficiencyToastHold alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (CalibrateInvitingPreloadScreen *)groupNickNameView{
    if(!_groupNickNameView){
        _groupNickNameView = [[CalibrateInvitingPreloadScreen alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (StopTreeMajor *)groupAlartView{
    if(!_groupAlartView){
        _groupAlartView = [[StopTreeMajor alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
