...
//    }
    ...
    self.header = [[EnvelopeVaporLockPositioned alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, self.view.width, 0)];
...
- (void)onTapMenuItemMark:(AnchorFocalHandlerDividerWall *)item
{
    ...
    [[NIMSDK sharedSDK].chatExtendManager addCollect:params completion:^(NSError * _Nullable error, NIMCollectInfo * _Nullable collectInfo) {
        if (error) {
            [ValidateCompositionInterpolationToward showErrorWithStatus:LangKey(@"收藏失败")];
            return;
        }
        [ValidateCompositionInterpolationToward showSuccessWithStatus:LangKey(@"收藏成功")];
    }];
}
...
- (void)onTapMenuItemTranslation:(AnchorFocalHandlerDividerWall *)item
{
    ...
    [ConfigPrimalCozy getWithUrl:Server_getNimCheckSum params:param isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
...
{
    if(!_translateView){
        _translateView = [[RoleClipRoyal alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (SaturationOrientationCleverView *)reprotHisNextView
{
    if(!_reprotHisNextView){
        _reprotHisNextView  = [[SaturationOrientationCleverView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
