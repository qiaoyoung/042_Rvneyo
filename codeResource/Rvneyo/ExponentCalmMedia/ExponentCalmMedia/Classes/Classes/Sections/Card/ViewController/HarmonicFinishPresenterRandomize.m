...
@interface HarmonicFinishPresenterRandomize ()<NIMUserManagerDelegate,PublishActivityProviderLimitDelegate,EasyJovialPetiteRenderDelegate,SaturationOrientationCleverDelegate>
...
@property (nonatomic, strong) SaturationOrientationCleverUserView *reprotView;
@property (nonatomic, strong) EffectDecorationLake *changeRemarksView;
...
@implementation HarmonicFinishPresenterRandomize
...
//    [self setUpNav];
    ...
    [ConfigPrimalCozy getWithUrl:Server_user_detail params:dict isShow:NO success:^(id responseObject) {
...
//    [ValidateCompositionInterpolationToward show];
    ...
    [ConfigPrimalCozy getWithUrl:Server_group_getgroups params:nil isShow:NO success:^(id responseObject) {
        [self.loadingView animationClose];
//        [ValidateCompositionInterpolationToward dismissWithCompletion:^{
            NSDictionary *resultDict = (NSDictionary *)responseObject;
            NSString *code = [resultDict newStringValueForKey:@"code"];
            if (code.integerValue == 0) {
                ...
                PublishActivityProviderLimit *dataPick = [[PublishActivityProviderLimit alloc] initWithDelegate:self dataDict:dataDict selectedDict:nil jsonNode:@"name"];
                dataPick.tag = 500;
                [dataPick show];
            }
//        }];
...
#pragma mark - PublishActivityProviderLimitDelegate
...
-(void)setingGroup{
    ...
    [ConfigPrimalCozy getWithUrl:Server_group_addgroupuser params:dict isShow:YES success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
...
- (EffectDecorationLake *)changeRemarksView
{
    if(!_changeRemarksView){
        _changeRemarksView = [[EffectDecorationLake alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (SaturationOrientationCleverUserView *)reprotView
{
    if(!_reprotView){
        _reprotView  = [[SaturationOrientationCleverUserView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
-(void)didTouchSubmitButton:(NSString *)reason
{
    NSMutableDictionary *dict = @{}.mutableCopy;
    dict[@"contact"] = reason?:@"";
    [ConfigPrimalCozy postWithUrl:Server_other_feedback params:dict isShow:NO success:^(id responseObject) {
    } failed:^(id responseObject, NSError *error) {
    }];
...
{
    if(!_reprotNextView){
        _reprotNextView  = [[SaturationOrientationCleverNextView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_reprotBlackView){
        _reprotBlackView  = [[SaturationOrientationCleverBlackView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (SaturationOrientationCleverDeleteView *)reprotDeleteView
{
    if(!_reprotDeleteView){
        _reprotDeleteView  = [[SaturationOrientationCleverDeleteView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
