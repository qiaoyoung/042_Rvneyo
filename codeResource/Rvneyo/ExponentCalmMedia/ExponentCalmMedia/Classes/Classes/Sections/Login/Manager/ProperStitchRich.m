...
+ (NSString *)deviceIUUID {
    if (iuuid.length <= 0) {
        iuuid = [ExportControlVideoCommit passwordForService:SNUserDefault_Key_IUUID account:SNKey];
        if (iuuid.length <= 0) {
...
+ (void)updateDeviceIUUID {
    ...
    CFRelease(uuidRef);
    [ExportControlVideoCommit setPassword:iuuid forService:SNUserDefault_Key_IUUID account:SNKey];
}
