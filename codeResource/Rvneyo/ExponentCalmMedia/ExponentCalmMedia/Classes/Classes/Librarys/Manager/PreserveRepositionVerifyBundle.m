...
+ (void)refreshRegistConfig{
    ...
    [ConfigPrimalCozy postWithUrl:Server_regist_config params:nil isShow:NO success:^(id responseObject) {
...
+ (void)firstSendRegistName:(NSString *)name pd:(NSString *)pd RequestWithComplete:(void(^)(BOOL sucess, NSString * msg))complete {
    ...
    [ConfigPrimalCozy postWithUrl:Server_first_regist_config params:@{@"username":name , @"passwd":pd } isShow:NO success:^(id responseObject) {
...
+ (void)sendRegistRequest:(UINavigationController *)nav{
    ...
    NSString *ismustmobile = [PreviewThemeAngularTriumph standardUserDefaults].ismustmobile;//是否手机

    if ([PreserveRepositionVerifyBundle shareConfigManager].headerImage == nil) {//默认头像
...
-(void)mobileRegist{
    ...
    if (!image) {
        ...
        [ConfigPrimalCozy uploadImagesWithURL:Server_regist_mobile parameters:dict images:@[UIImageJPEGRepresentation(image, 0.3)] progress:^(NSProgress *progress) {
        ...
        [ConfigPrimalCozy getWithUrl:Server_regist_mobile params:dict isShow:YES success:^(id responseObject) {
            NSDictionary *resultDict = (NSDictionary *)responseObject;
            NSString *code = [resultDict newStringValueForKey:@"code"];
            NSString *msg = [resultDict newStringValueForKey:@"msg"];
            [ValidateCompositionInterpolationToward showMessage:msg];
...
-(void)accountRegist{
    ...
    if (image) {
        [ConfigPrimalCozy uploadImagesWithURL:Server_regist_regist parameters:dict images:@[UIImageJPEGRepresentation(imageForAvatarUpload, 0.7)] progress:^(NSProgress *progress) {
            ...
            [ValidateCompositionInterpolationToward showMessage:LangKey(@"register_avtivity3_register_fail")];
