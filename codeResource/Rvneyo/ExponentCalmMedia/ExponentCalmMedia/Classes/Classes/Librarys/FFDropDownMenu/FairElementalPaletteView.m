    ...
    if (_tableView == nil) {
        ...
        if ([self.cellClassName hasSuffix:@".xib"]) { //xib名称
            ...
            if (![NSClassFromString(className) isSubclassOfClass:[FairElementalPaletteBasedCell class]]) {
                FFLog(@"%@这个类不是FairElementalPaletteBasedCell的子类,请继承这个类",className);
                return _tableView;
            }
            ...
            if (![NSClassFromString(self.cellClassName) isSubclassOfClass:[FairElementalPaletteBasedCell class]]) {
                FFLog(@"%@这个类不是FairElementalPaletteBasedCell的子类,请继承这个类",self.cellClassName);
                return _tableView;
            }
...
- (void)setMenuAnimateType:(FairElementalPaletteViewAnimateType)menuAnimateType { //15
