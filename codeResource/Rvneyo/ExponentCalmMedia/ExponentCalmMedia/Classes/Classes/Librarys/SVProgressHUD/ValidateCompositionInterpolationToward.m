...
#error ValidateCompositionInterpolationToward is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif
...
+ (ValidateCompositionInterpolationToward*)sharedView {
    ...
    static ValidateCompositionInterpolationToward *sharedView;
     ...
     dispatch_once(&once, ^{ sharedView = [[self alloc] initWithFrame:[ValidateCompositionInterpolationToward mainWindow].bounds]; });
#else
...
- (void)updateHUDFrame {
    ...
    hudWidth = ValidateCompositionInterpolationTowardHorizontalSpacing + MAX(labelWidth, contentWidth) + ValidateCompositionInterpolationTowardHorizontalSpacing;
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(ValidateCompositionInterpolationTowardVerticalSpacing, (self.minimumSize.height - contentHeight - ValidateCompositionInterpolationTowardLabelSpacing - labelHeight) / 2.0f);
...
#else
            // If ValidateCompositionInterpolationToward is used inside an app extension add it to the given view
