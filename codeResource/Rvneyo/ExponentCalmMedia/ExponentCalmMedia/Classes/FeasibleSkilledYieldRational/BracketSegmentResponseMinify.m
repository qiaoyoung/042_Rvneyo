...
@interface TreatVideo_Data : NSObject

@end

@implementation TreatVideo_Data

//: online_state
+ (NSString *)dataGraphDate {
    /* static */ NSString *dataGraphDate = nil;
    if (!dataGraphDate) {
		NSString *origin = @"0c17052ecc86858380857c768a8b788b7c82";
		NSData *data = [TreatVideo_Data TreatVideo_DataToData:origin];
        Byte *value = (Byte *)data.bytes;
        dataGraphDate = [self StringFromTreatVideo_Data:value];
    }
    return dataGraphDate;
}

//: net_state
+ (NSString *)screenPushToken {
    /* static */ NSString *screenPushToken = nil;
    if (!screenPushToken) {
		NSString *origin = @"09450784b70787b3aab9a4b8b9a6b9aa99";
		NSData *data = [TreatVideo_Data TreatVideo_DataToData:origin];
        Byte *value = (Byte *)data.bytes;
        screenPushToken = [self StringFromTreatVideo_Data:value];
    }
    return screenPushToken;
}

+ (Byte *)TreatVideo_DataToCache:(Byte *)data {
    int driftBelow = data[0];
    Byte powerComplexClassify = data[1];
    int lineTolerance = data[2];
    for (int i = lineTolerance; i < lineTolerance + driftBelow; i++) {
        int value = data[i] - powerComplexClassify;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[lineTolerance + driftBelow] = 0;
    return data + lineTolerance;
}

+ (NSString *)StringFromTreatVideo_Data:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self TreatVideo_DataToCache:data]];
}

+ (NSData *)TreatVideo_DataToData:(NSString *)value {
...
// __CLOSE_PRINT__
//
//  BracketSegmentResponseMinify.m
//  NIM
//
...
//: @interface BracketSegmentResponseMinify()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
@interface BracketSegmentResponseMinify()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
@property (nonatomic,strong) NSMutableSet *local;

//: @property (nonatomic,strong) NSMutableSet *tempSubscribeIds;
@property (nonatomic,strong) NSMutableSet *stormFail;

//: @end
@end

//: @implementation BracketSegmentResponseMinify
@implementation BracketSegmentResponseMinify

//: - (void)subscribeTempSaveGenerateThornGroupState:(NSString *)userId
- (void)will:(NSString *)userId
{
    ...
    NIMSubscribeRequest *request = [self organic];
    //: request.publishers = @[userId];
    request.publishers = @[userId];
    //: [self.tempSubscribeIds addObject:userId];
    [self.stormFail addObject:userId];
              ...
              totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P && ![self.heap containsObject:recentSession.session.sessionId]) {
        //: [self.subscribeIds removeObject:recentSession.session.sessionId];
        [self.local removeObject:recentSession.session.sessionId];
    }
}


//: - (void)publishOnlineState
- (void)backgroundImplement
{
    //: NIMSubscribeEvent *event = [[NIMSubscribeEvent alloc] init];
    NIMSubscribeEvent *event = [[NIMSubscribeEvent alloc] init];
    //: event.type = NIMSubscribeSystemEventTypeOnline;
    event.type = NIMSubscribeSystemEventTypeOnline;
    //: event.value = ElevatorWinterNamespaceValueOnlineExt;
    event.value = ElevatorWinterNamespaceValueOnlineExt;
    //: event.sendToOnlineUsersOnly = NO; 
    event.sendToOnlineUsersOnly = NO; //必须要让后登录的用户也能拿到    
    //: NSDictionary *ext = @{@"net_state" : @([PastGesturePassive currentDevice].currentNetworkType),
    NSDictionary *ext = @{[TreatVideo_Data screenPushToken] : @([PastGesturePassive efficiency].previous),
                          //: @"online_state" : @(SaveGenerateThornGroupStateNormal), //移动端永远在线
                          [TreatVideo_Data dataGraphDate] : @(SaveGenerateThornGroupStateNormal), //移动端永远在线
                          //: };
                          };
    //: [event setExt:[ext nimkit_jsonString]];
    [event setExt:[ext tensionBy]];
           ...
           totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P) {
        ...
        [self.local addObject:recentSession.session.sessionId];
...
{
    //: static BracketSegmentResponseMinify *instance;
    static BracketSegmentResponseMinify *instance;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
    //: _dispatch_once(&onceToken, ^{
    _dispatch_once(&onceToken, ^{
        //: instance = [[BracketSegmentResponseMinify alloc] init];
        instance = [[BracketSegmentResponseMinify alloc] init];
    //: });
    });
    //: return instance;
...
- (void)transactionMaster:(NSArray *)ids
{
    //: if (!ids.count) {
    if (!ids.count) {
        //: return;
    ...
    [[NIMSDK sharedSDK].subscribeManager subscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
        ...
        if (members.count) {
            ...
            [weakSelf transactionMaster:members];
        }
    //: }];
    }];
}
...
- (instancetype)init
{
    ...
    if (self)
    {
        ...
        _local = [[NSMutableSet alloc] init];
        //: _tempSubscribeIds = [[NSMutableSet alloc] init];
        _stormFail = [[NSMutableSet alloc] init];
...
- (void)span:(NSString *)userId
{
    ...
    if (![_local containsObject:userId])
    {
        ...
        [self.stormFail removeObject:userId];
    }
}
...
- (void)onRecvSubscribeEvents:(NSArray *)events
{
    ...
    for (NIMSubscribeEvent *event in events) {
        ...
        if ([self.local containsObject:event.from] || [self.stormFail containsObject:event.from])
        {
            //: NSInteger type = event.type;
            NSInteger type = event.type;
            //: NSMutableDictionary *eventsDict = [self.events objectForKey:@(type)];
            NSMutableDictionary *eventsDict = [self.next objectForKey:@(type)];
            //: if (!eventsDict) {
            if (!eventsDict) {
                //: eventsDict = [[NSMutableDictionary alloc] init];
                eventsDict = [[NSMutableDictionary alloc] init];
                //: [self.events setObject:eventsDict forKey:@(type)];
                [self.next setObject:eventsDict forKey:@(type)];
            }
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (isMyFriend && ![self.local containsObject:user.userId])
    {
        //是好友却没订阅，订阅一下
        //: NIMSubscribeRequest *request = [self generateRequest];
        NIMSubscribeRequest *request = [self organic];
